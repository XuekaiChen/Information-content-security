...
class MyData(data.Dataset):
	...
	def split(cls, text_field, label_field, args, state, shuffle=True, **kwargs):
		if state is 'train':
			...
			examples = cls(text_field, label_field, cover_path=cover_path, stego_path=stego_path).examples
			if shuffle: random.shuffle(examples)
			import math
			val_idx = -math.floor(0.05 * len(examples))
			return cls(text_field, label_field, examples=examples[:val_idx]), cls(text_field, label_field, examples=examples[val_idx:])
